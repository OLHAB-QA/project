...
BEGIN
    INSERT INTO logs(id, appl_proc, message) VALUES(log_seq.NEXTVAL, p_appl_proc, p_message);
COMMIT;
END to_log;
...
BEGIN
    log_util.log_start(p_proc_name => 'my_process');
END;


BEGIN
    log_util.log_finish(p_proc_name => 'my_process');
END;



BEGIN
    log_util.log_error(p_proc_name => 'my_process', p_sqlerrm => SQLERRM);
END;
