...
create or replace PACKAGE BODY util_p AS
  ...
  BEGIN
    ...
    -- Перевірка, чи існує код посади в таблиці JOBS
    SELECT COUNT(*)
    INTO v_job_exists
    FROM JOBS
    ...
    -- Перевірка, чи існує ідентифікатор відділу в таблиці DEPARTMENTS
    SELECT COUNT(*)
    ...
    -- Перевірка діапазону зарплати для коду посади
    ...
    -- Генерація нового EMPLOYEE_ID
    SELECT NVL(MAX(EMPLOYEE_ID), 0) + 1
    INTO v_employee_id
    FROM EMPLOYEES;

    -- Додавання співробітника до таблиці EMPLOYEES
    BEGIN
